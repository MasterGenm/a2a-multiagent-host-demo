...
此脚本将复杂的 Tavily 搜索功能分解为一系列目标明确、参数极少的独立工具，
专为 AI Agent 调用而设计。Agent 只需根据任务意图选择合适的工具，
...
# ✅ 稳健：使用包内相对导入（不再修改 sys.path）
from ..utils.retry_helper import with_graceful_retry, SEARCH_API_RETRY_CONFIG  # type: ignore

# 运行前请确保已安装 Tavily 库: pip install tavily-python
try:
    from tavily import TavilyClient  # type: ignore
except ImportError as e:
    raise ImportError("Tavily 库未安装，请先运行：pip install tavily-python") from e
...
class TavilyResponse:
    """封装 Tavily API 的完整返回结果，以便在工具间传递"""
    query: str
...
class TavilyNewsAgency:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        初始化客户端。
        Args:
            api_key: Tavily API 密钥，若不提供则从环境变量 TAVILY_API_KEY 读取。
        """
        key = api_key or os.getenv("TAVILY_API_KEY")
        if not key:
            raise ValueError("Tavily API Key 未找到！请设置环境变量 TAVILY_API_KEY 或在初始化时传入 api_key")
    ...
    def _search_internal(self, **kwargs: Any) -> TavilyResponse:
        """
        内部通用的搜索执行器，所有工具最终都调用此方法。
        - 统一强制 topic='news'（与工具设计一致）
        - 自动清理 None 参数
        """
        try:
            # ✅ 统一默认新闻语料
            ...
            response_dict: Dict[str, Any] = self._client.search(**api_params)  # type: ignore
    ...
    def basic_search_news(self, query: str, max_results: int = 7) -> TavilyResponse:
        ...
        Agent 可提供搜索查询 (query) 和可选的最大结果数 (max_results)。
        """
    ...
    def deep_search_news(self, query: str) -> TavilyResponse:
        ...
        返回 AI 生成的“高级”详细摘要答案和最多 20 条最相关的新闻结果。适用于需要全面了解某个事件背景的场景。
        Agent 只需提供搜索查询 (query)。
        """
        ...
        return self._search_internal(
            ...
            max_results=20,
            include_answer="advanced",  # tavily 支持 True/False/'advanced'
        )

    def search_news_last_24_hours(self, query: str) -> TavilyResponse:
        """
        【工具】搜索 24 小时内新闻: 获取关于某个主题的最新动态。
        此工具专门查找过去 24 小时内发布的新闻。适用于追踪突发事件或最新进展。
        Agent 只需提供搜索查询 (query)。
        """
    ...
    def search_news_by_date(self, query: str, start_date: str, end_date: str) -> TavilyResponse:
        ...
        这是唯一需要 Agent 提供详细时间参数的工具。
        需要提供查询 (query)、开始日期 (start_date) 和结束日期 (end_date)，格式均为 'YYYY-MM-DD'。
        """
...
def print_response_summary(response: TavilyResponse) -> None:
    """简化的打印函数，用于展示测试结果（会显示发布日期）"""
    ...
    if response.answer:
        print(f"AI 摘要: {response.answer[:120]}...")
    print(f"找到 {len(response.results)} 条网页, {len(response.images)} 张图片。")
    if response.results:
        first = response.results[0]
        date_info = f"(发布于: {first.published_date})" if first.published_date else ""
        print(f"第一条结果: {first.title} {date_info}")
    print("-" * 60)


if __name__ == "__main__":
    # 在运行前，请确保已设置 TAVILY_API_KEY 环境变量
    try:
        agency = TavilyNewsAgency()
        r1 = agency.basic_search_news(query="奥运会最新赛况", max_results=5)
        print_response_summary(r1)

        r2 = agency.deep_search_news(query="全球芯片技术竞争")
        print_response_summary(r2)

        r3 = agency.search_news_last_24_hours(query="Nvidia GTC 大会 最新发布")
        print_response_summary(r3)

        r4 = agency.search_news_last_week(query="自动驾驶商业化落地")
        print_response_summary(r4)

        r5 = agency.search_images_for_news(query="韦伯太空望远镜最新发现")
        print_response_summary(r5)

        r6 = agency.search_news_by_date(
            query="人工智能法规", start_date="2025-01-01", end_date="2025-03-31"
        )
        print_response_summary(r6)
